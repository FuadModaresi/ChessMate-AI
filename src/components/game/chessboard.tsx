    ...
    return (
        <div className="relative" style={{ paddingBottom: '1.5rem', paddingLeft: '1.5rem' }}>
            <div className="aspect-square w-full grid grid-cols-8 grid-rows-8 border-2 border-primary/20 shadow-inner rounded-md overflow-hidden">
                {viewRanks.flatMap((rank, rowIndex) =>
                    viewFiles.map((file, colIndex) => {
                        ...
                        return (
                            <div
                                key={square}
                                className={cn(
                                    "flex justify-center items-center relative transition-colors duration-200",
                                    isDark ? "bg-primary/80" : "bg-secondary",
             ...
             <div className="absolute top-0 bottom-[1.5rem] left-0 w-[1.5rem] flex flex-col-reverse justify-around items-center text-primary/70 font-bold text-sm">
                {viewRanks.map(rank => <span key={`rank-${rank}`}>{rank}</span>)}
            </div>
            <div className="absolute bottom-0 left-[1.5rem] right-0 h-[1.5rem] flex justify-around items-center text-primary/70 font-bold text-sm">
